...
def answer_index(system, topic, message_content, temp):
    ...
    completion = openai.ChatCompletion.create(
        model="no need anymore", messages=messages, temperature=temp
    )
...
def answer_user_question(topic):
    system = (
        ...
        "занимающейся логистикой, тебя зовут "
        "пекGPT и ты отвечаешь на вопросы "
        "сотрудников в чате. У тебя есть документ "
        "со всеми материалами о внутренних процессах "
        "и регламентах. Ответь так, чтобы сотрудник "
        "решил свой вопрос. То чего нет в документе "
        "мы не можем это ответить. Не употребляй "
        "фразы вида “По нашему документу”, “в нашем "
        "документе”, “в документе с материалами”."
    )
    # Ищем реливантные вопросу чанки и формируем контент для модели,
    # который будет подаваться в user
